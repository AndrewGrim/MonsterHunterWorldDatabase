...
class PreferencesWindow:
	...
	def __init__(self, root, preferences):
		...
		self.selectTab = wx.Choice(panel, choices=tabs, size=(200, 30))
		self.selectTab.SetSelection(self.selectTab.FindString(self.pref.initialTab))

		self.rememberSize = wx.CheckBox(panel, label="Remember window size", size=(200, 30))
		self.rememberSize.SetValue(self.pref.rememberSize)
		self.rememberPosition = wx.CheckBox(panel, label="Remember window position", size=(200, 30))
		self.rememberPosition.SetValue(self.pref.rememberPosition)
		self.unicodeSymbols = wx.CheckBox(panel, label="Use unicode symbols", size=(200, 30))
		self.unicodeSymbols.SetValue(self.pref.unicodeSymbols)
		self.keepSearchOpen = wx.CheckBox(panel, label="Keep search window open", size=(200, 30))
